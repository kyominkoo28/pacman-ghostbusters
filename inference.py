    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.beliefs:
            ...
            for p2 in newPosDist:
                allPossible[p2] += (newPosDist[p2] *self.beliefs[p])
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for p in beliefs:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for p2 in newPosDist:
                allPossible[p2] += (newPosDist[p2] * beliefs[p])
        allPossible.normalize()
        for i in range(len(self.particles)):
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for firsts in a :
            for seconds in b:
                tuples = (firsts,seconds)
                lst.append(tuples)
        new = []
        for elements in lst:
            if not elements[0]==elements[1]:
                new.append(elements)
        self.particles = []
        n = 0
        while n < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            allPossible[p] += (emissionModels[0][trueDistance0] * emissionModels[1][trueDistance1])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newGhostPos = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
